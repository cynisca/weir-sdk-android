...
/**
 * R2-R2: two rapid [Weir.present] calls must not stack two overlapping flow
 * Activities. The second call, made
...
class WeirPresentGuardTest {
    ...
    fun secondPresentWhileFirstInFlight_isRejectedNotStacked() {
        ...
        Weir.present(context = activity, flowId = "cob_intake", eventSink = LoggingEventSink()) { results.add(it) }
        Weir.present(context = activity, flowId = "cob_intake", eventSink = LoggingEventSink()) { results.add(it) }
