...
/**
 * Request body shape for a flush batch.
 *
 ...
 * in a `{"appId", "events"}` envelope so the server doesn't have to infer appId
 * from the write token alone. Each [EventParams]'s own on-the-wire JSON shape
 * (flowId/screenId/variantId/sessionId/userId/ts/seq/payload) is exactly what
 * [EventQueue] persists to disk — only this top-level wrapper is added here.
 */
@Serializable
...
class HttpUrlConnectionEventIngestUploader(
        ...
        withContext(Dispatchers.IO) {
            val body: ByteArray = try {
                WeirJson.encodeToString(
                    ...
                    FlushRequestBody(appId = config.appId, events = events),
            ...
            try {
                ...
                // Drain the response stream so the connection can be pooled and
                // released cleanly (HttpURLConnection keep-alive requires it).
                drainQuietly(connection)
                // 2xx only counts as success. Anything else (4xx/5xx) is a
                // failure the queue retains and retries — server downtime must
                // mean delayed events, never lost.
                val success = status in 200..299
                EventUploadOutcome(success = success, httpStatus = status)
            } catch (e: Exception) {
    ...
    private fun drainQuietly(connection: HttpURLConnection) {
        try {
            val stream = if (connection.responseCode in 200..299) connection.inputStream else connection.errorStream
            stream?.bufferedReader()?.use(BufferedReader::readText)
        } catch (_: Exception) {
            // Best-effort drain only; the response code is already read.
        }
    }
}
